...
class Rule:
    ...
    def _already_acted(self, fullname, mod):
        ...
        self.cursor.execute(
            "SELECT 1 FROM actions INNER JOIN users "
        ...
        if result:
            ...
            self.cursor.execute(
                "SELECT users.username FROM actions "
                "INNER JOIN users "
                "ON actions.moderator = users.id "
                "WHERE target_type = ? AND target_id = ?",
    ...
    def match(self, report, thing):
        ...
        return any(
            report.casefold() == command for command in self.commands
    ...
    def log_action(self, target, moderator):
        ...
        self.cursor.execute(
            "SELECT id FROM users WHERE username = ?", (author_name,)
        )
        author_id = self.cursor.fetchone()[0]
        self.cursor.execute(
            "INSERT OR IGNORE INTO users (username) " "VALUES(?)", (moderator,)
        )
        self.cursor.execute(
        ...
        print(
            moderator, action_summary, action_details, target, self.subreddit
        )
    ...
    def validate_params(cls, params):
        ...
        for param, value in params.items():
            ...
            if value and not isinstance(value, required_type):
                raise RuntimeError(
                    "Invalid type of parameter {} (expected {})".format(
                        param, required_type
                    )
                )
...
class Banner(Actor):
    ...
    def __init__(
        self, message=None, reason=None, duration=None, *args, **kwargs
    ):
    ...
    def action(self, post, mod):
        ...
        try:
            self.subreddit.modmail.create(
                self.subject, self.body, post.author, author_hidden=True
            )
        except prawcore.PrawcoreException as exception:
            logging.error(
                "Failed to send modmail on %s: %s", post.name, exception
            )
            return
...
class Notifier(Actor):
    ...
    def action(self, post, mod):
        ...
        try:
            ...
            logging.error(
                "Failed to add comment on %s: %s", post.name, exception
            )
            return
        ...
        try:
            result.mod.distinguish(
                sticky=isinstance(post, praw.models.Submission)
            )
        except prawcore.PrawcoreException as exception:
            logging.error(
                "Failed to distinguish comment on %s: %s", post.name, exception
            )
    ...
    def after(self):
        ...
        for _ in range(30):
            ...
            try:
                ...
                logging.error(
                    "Failed to remove comment %s: %s", comment.name, exception
                )
...
class Nuker(Actor):
    ...
    def action(self, post, mod):
        ...
        try:
            ...
            logging.error(
                "Failed to retrieve comment tree on %s: %s",
                post.name,
                exception,
            )
            return
...
class ToolboxNoteAdderActionBuffer(ActionBuffer):
    ...
    def _transform_page(self, content):
        ...
        if usernotes_dict["ver"] != self.EXPECTED_VERSION:
            logging.error(
                "Unexpected toolbox notes version: %s", usernotes_dict["ver"]
            )
            raise RuntimeError
        ...
        for note in self.notes:
            serializable_note = note.to_serializable(
                mod_indices, warning_indices
            )
